...
void main() {
  ...
  group('MockApplicationRepository Tests', () {
    ...
    test('getMyApplications filters by status correctly (APP-01-03)', () async {
      ...
      expect(
        reviewedList.every((app) => app.status == ApplicationStatus.reviewed),
        true,
      );
      ...
      expect(
        acceptedList.every((app) => app.status == ApplicationStatus.accepted),
        true,
      );
    });
    ...
    test('getApplicationById retrieves correct application or null', () async {
      ...
      final nonExistent = await repository.getApplicationById(
        'non-existent-id',
      );
    ...
    test(
      'hasApplied returns true for seeded jobs and false for new jobs',
      () async {
    ...
    test(
      'applyJob successfully submits new application (APP-01-01, MOB-01-04)',
      () async {
    ...
    test(
      'applyJob prevents duplicate applications for the same job (APP-01-02)',
      () async {
