...
//   health -> register/login -> job search -> apply (if jobs exist)
//
...
//
// When the gateway is unreachable the test skips instead of failing, so
// `flutter test` in CI without a live stack stays green.
...
void main() {
  ...
  testWidgets('gateway E2E: health -> auth -> search -> apply', (tester) async {
    ...
    expect(jobs.items, isA<List<Object>>());

    if (jobs.items.isNotEmpty) {
      final job = jobs.items.first;
      final applications = ApiApplicationRepository(
